...
\section{Einführende Beispiele}
	...
	\begin{aufgabe}[Stratos Jump]
		Am $14$. Oktober $2012$ sprang Felix Baumgartner von $h=38'969$ Metern in die Tiefe. Wir möchten seinen Sprung als freien Fall ohne Widerstand modellieren. Er beschleunigte also konstant mit der Erdbeschleunigung $g=9.81\frac{m}{s^2}$.\vspace{5pt}\\
...
\section{Der Weg zum bestimmten Integral}
	\begin{wrapfigure}{r}{0.4\textwidth}
		\centering\includegraphics[width=0.3\textwidth]{x2_ausgang.png}
	\end{wrapfigure}
	...
	\begin{minipage}{0.8\linewidth}
		\textbf{Untersumme:}
		\centering\includegraphics[width=0.7\textwidth]{x2_USumme4.png}\\
	...
	\begin{minipage}{0.8\linewidth}
		\textbf{Obersumme:}
		\centering\includegraphics[width=0.7\textwidth]{x2_OSumme4.png}\\
	...
	\begin{minipage}{0.8\linewidth}
		\centering\includegraphics[width=0.7\textwidth]{x2_USumme8.png}\\
	...
	\hfill
	\begin{minipage}{0.8\linewidth}
		\centering\includegraphics[width=0.7\textwidth]{x2_OSumme8.png}\\
		Die Höhe der Rechtecke ist also:
		\begin{enumerate}[label=\ ]\begin{multicols}{2}
				\item $f(\frac{1}{4})=\frac{1}{16}$
				\item $f(\frac{1}{2})=\frac{1}{4}$
	...
	Natürlich können wir jetzt die die Anzahl Rechtecke erneut verdoppeln und so eine noch genauere Abschätzung erhalten. Wir sehen dann:\\
	\begin{minipage}{0.8\linewidth}
		\begin{align*}
	...
	\begin{aufgabe}
		\begin{enumerate}
			\item Betrachte die Funktion $f(x)=x$ im Bereich $a=0$ und $b=1$.
			\begin{enumerate}[label=\alph*)]
			...
			\end{enumerate}
			\item Betrachte die Funktion $f(x)=\frac{1}{2}x+2$ im Bereich $a=1$ und $b=4$.
			\begin{enumerate}[label=\alph*)]
				\item Teile das Intervall in $5$ gleich breite Teile und berechne die Fläche unter $f$ mithilfe der Ober- und Untersumme.
				...
				\item Da $f$ eine lineare Funktion ist, sind wir in der Lage, die Fläche exakt berechnen. Vergleiche deine Lösung mit den Annäherungen aus $a)$.
			\end{enumerate}
			\item Betrachte die Funktion $f(x)=\frac{1}{x}$ im Bereich $a=2$ und $b=4$.\vspace*{11pt}\\
			\begin{minipage}{0.55\linewidth}
				\begin{enumerate}[label=\alph*)]
			...
			\begin{minipage}{0.4\linewidth}
				\centering\includegraphics[scale=0.25]{aufg_OberUnter.png}
			\end{minipage}
		\end{enumerate}
	\end{aufgabe}
	...
	\begin{definition}[bestimmtes Integral]
		...
		\textbf{TR:} Menü $\to\:4\:\to\:3$
	\end{definition}
	Dabei müssen wir aber folgende Spezialfälle sauber auseinanderhalten:
	\begin{enumerate}[label=\Alph*)]
		\item Spezialfall $a=b$:\\ Falls die beiden Grenze zusammenfallen, so ist die Breite der Rechtecke $\gap{2}$ und damit sind sowohl die Ober- als auch die Untersumme ebenfalls gleich $0$. Das heisst: \[\gap{5}.\]
		\item Spezialfall $a>b$:\\ Falls $a$ grösser ist als $b$, so gilt für die Breite der Rechtecke $\gap{2}$ und damit ändert sich auch das Vorzeichen des Integrals \[\gap{5}.\]
	\end{enumerate}
	\begin{satz}
		Satz vertauschen der Integrationsgrenzen und aufteilen der Integrationsgrenzen...
	\end{satz}
	\begin{aufgabe}
...
\section{Die Stammfunktion}
	...
	\begin{aufgabe}
		\begin{enumerate}
			\item Suche für die folgenden Polynome $f$ eine Funktion $F$, deren Ableitung $F'=f$ ist.
			\begin{enumcols}[3]
				\item $f(x)=x$
				\item $f(x)=x^2$
				\item $f(x)=x^3$
			...
			\item Suche auch für die folgenden speziellen Funktionen eine Stammfunktion zu finden.
			\begin{enumcols}[3]
				\item $f(x)=\e^x$
				\item $f(x)=\sqrt{x}$
				\item $f(x)=\frac{1}{x}$
	...
	\end{notation}
	Die Rechenregeln für die Integrale sind im Grunde die gleichen wie fürs Ableiten. Ausgenommen davon sind die Produkt-, Quotienten- und Kettenregel.
	...
	\end{satz}
	\begin{aufgabe}
		Bestimme im folgenden alle unbestimmten Integrale.
		\begin{enumerate}
			\item Polynomfunktionen
			\begin{enumcols}[2]
				\item $\displaystyle\int4x\d{x}$
				\item $\displaystyle\int1\d{x}$
				\item $\displaystyle\int(3x^2-1)\d{x}$
				\item $\displaystyle\int(x^4+x^3+x^2+x+1)\d{x}$
				\item $\displaystyle\int\left(5x^4+\frac{1}{2}x^3-\frac{7}{3}x^2\right)\d{x}$
				\item $\displaystyle\int(3x+4)(x^2-1)\d{x}$
				\item $\displaystyle\int(3x^2-1)^2\d{x}$
				\item $\displaystyle\int\left(\frac{3}{x^3}+\frac{5}{x^2}-\frac{1}{4}\right)\d{x}$
			\end{enumcols}
			\item Wurzelfunktionen
			\begin{enumcols}[2]
				\item $\displaystyle\int\sqrt{x}\d{x}$
				\item $\displaystyle\int\sqrt{x}\*(x+5)\d{x}$
				\item $\displaystyle\int\left(x+\sqrt[4]{x}\right)\d{x}$
				\item $\displaystyle\int\left(\sqrt[4]{x^6}+x^3-2\right)\d{x}$
			\end{enumcols}
			\item Winkelfunktionen
			\begin{enumcols}[2]
				\item $\displaystyle\int\cos{x}\d{x}$
				\item $\displaystyle\int\left(2\*\sin{x}-3\*\cos{x}\right)\d{x}$
				\item $\displaystyle\int\left(x-\cos{x}\right)\d{x}$
				\item $\displaystyle\int\left(4\*\sin{x}-7\*\cos{x}\right)\d{x}$
				\item $\displaystyle\int\left(1+(\tan{x})^2\right)\d{x}$
				\item $\displaystyle\int\left(\exp{x}+\sin{x}\right)\d{x}$
			\end{enumcols}
			\item Exponential- und Logarithmusfunktionen
			\begin{enumcols}[2]
				\item $\displaystyle\int 7\*\e^x\d{x}$
				\item $\displaystyle\int5^x\d{x}$
				\item $\displaystyle\int\frac{1}{x}\*\sqrt{x}\d{x}$
				\item $\displaystyle\int\pi\*\ln{x}\d{x}$
				\item $\displaystyle\int\left(\frac{1}{3}\*\exp{x}+\frac{1}{x}\right)\d{x}$
				\item $\displaystyle\int\left(\frac{3}{x}-\frac{4}{x^2}\right)\d{x}$
			\end{enumcols}
		\end{enumerate}
	\end{aufgabe}

\subsection*{Zusammenfassung}
	Alle Regeln zur Bildung einer Stammfunktion sind in einer übersichtlichen Tabelle in der Formelsammlung zusammengestellt. Der Vorgang um eine Spalte nach rechts heisst \glqq ableiten\grqq\: und in die Gegenrichtung, also nach links, heisst \glqq integrieren\grqq. (vlg. Formelsammlung Seite $29-30$)
	\newpage
...
\section{Der Hauptsatz der Differential und Integralrechnung}
	...
	\begin{beweis}
		\begin{wrapfigure}{r}{0.4\textwidth}
			\centering\includegraphics[width=0.3\textwidth]{flächenfunktion.png}
		\end{wrapfigure}
	...
	\end{beispiel}
	\begin{aufgabe}
		Berechne die folgenden bestimmten Integrale mit Hilfe des Hauptsatzes von Hand.
		\begin{enumcols}[4]
			\item $\displaystyle\int_0^2 3\*x^3\d{x}$
			\item $\displaystyle\int_1^3 x^3\d{x}$
			\item $\displaystyle\int_2^4\left(x^2+3\right)\d{x}$
			\item $\displaystyle\int_1^3\left(x^2-4x\right)\d{x}$
			\item $\displaystyle\int_0^4\sqrt{x}\d{x}$
			\item $\displaystyle\int_0^1 5x\*\sqrt{x}\d{x}$
			\item $\displaystyle\int_0^{\frac{\pi}{2}}\cos{x}\d{x}$
			\item $\displaystyle\int_0^\pi\sin{x}\d{x}$
			\item $\displaystyle\int_1^2\left(2x-\frac{2}{x}\right)\d{x}$
			\item $\displaystyle\int_1^3\left(4x-\frac{3}{x}+\frac{2}{x^2}\right)\d{x}$
			\item $\displaystyle\int_1^2\left(\frac{4}{x}+\frac{5}{x^2}+\frac{6}{x^3}\right)\d{x}$
			\item $\displaystyle\int_1^2\left(\e^x+\frac{1}{x}\right)\d{x}$
		\end{enumcols}
	\end{aufgabe}
	\newpage
	...
	\begin{aufgabe}
		...
		\begin{table}[H]
			...
			\begin{tabular}{p{7cm} c}
				\includegraphics[scale=0.3]{flächePosNeg1.png} & \includegraphics[scale=0.3]{flächePosNeg2.png} \\
				\includegraphics[scale=0.3]{flächePosNeg3.png} & \includegraphics[scale=0.3]{flächePosNeg4.png}
			\end{tabular}
		\end{table}
		\begin{enumerate}[label=\alph*)]
			\item Berechne für jeden oben abgebildeten Graphen die eingezeichnete Fläche.\vspace{5pt}\\\karos{6}
			\item Erkläre das Problem, welches du feststellst.\vspace{5pt}\\\karos{2}
			\item Suche nach einer möglichen Lösung und beschreibe dein Lösungsweg möglichst genau. \textit{Tipp: Nullstellen}\vspace{5pt}\\\karos{2}
		\end{enumerate}
	\end{aufgabe}
	\begin{algo}
...
\subsection{Fläche zwischen zwei Graphen}
	...
	\begin{aufgabe}
		...
		\begin{enumerate}[label=\alph*)]
			\item Wie lässt sich die Länge der senkrechten orangen Strecken mathematisch beschrieben.\vspace{5pt}\\\karos{1}
			\item Verschiebe nun die Fläche, sodass sie direkt über der $x-$Achse liegt. Was beschreibt diese Funktion?\vspace{5pt}\\\karos{1}
			\item Wir definieren die Differenzfunktion $f(x)-g(x)$. Welche Fläche wird nun von dieser Differenzfunktion und der $x-$Achse eingeschlossen?\vspace{5pt}\\\karos{1}
			\item Schliesse daraus eine Formel zur Berechnung der Fläche zwischen den beiden Graphen und wende sie auf dieses Beispiel an.\vspace{5pt}\\\karos{5}
		\end{enumerate}
	\end{aufgabe}
...
\subsection{Volumen eines Rotationskörpers}
	...
	\begin{aufgabe}
		...
		\begin{enumerate}[label=\alph*)]
			...
			\begin{figure}[H]
				\includegraphics[scale=0.1]{zylinder.jpeg}
			\end{figure}
	...
	\end{satz}
	Zusätzliche Infos zum Grenzwertprozess: \href{https://www.geogebra.org/m/xqaurwan}{Geogebra-Applet}\footnote{\url{https://www.geogebra.org/m/xqaurwan}}
	\begin{aufgabe}
	...
	\begin{aufgabe}[für die Schnellen]
		...
		\begin{minipage}{0.35\linewidth}
			\begin{figure}[H]
				\centering\includegraphics[scale=0.5]{keplerfass.jpeg}
			\end{figure}
		\end{minipage}
	\end{aufgabe}


\include{erweiterung-integral.tex}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{13mm}|p{20mm}|p{30mm}|p{70mm}|}\hline
		...
		$1.0$ & 02.04.25 & Jonas Guler & Kapitel erstellt \\\hline
		$1.1$ & 20.11.25 & Jonas Guler & Fehler verbessert \\\hline
		to Do & 03.12.25 & Jonas Guler & \\\hline
	\end{tabular}
\end{table}
\endgroup
