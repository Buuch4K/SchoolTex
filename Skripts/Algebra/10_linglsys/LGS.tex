...
\subsection{Das Additionsverfahren}
	...
	\begin{beispiel}
		...
		\begin{minipage}{0.45\linewidth}
			\centering\includegraphics[width=6cm]{images/waage_x-4y=8.png}
		\end{minipage}
		\hfill
		\begin{minipage}{0.45\linewidth}
			\centering\includegraphics[width=6cm]{images/waage_3x+4y=12.png}
		\end{minipage}
...
\subsection{Zur Anwendung der verschiedenen Verfahren}
	...
	\begin{aufgabe}
		\begin{enumerate}
			\item Wähle selbst ein geeignetes Verfahren zur Lösung der folgenden linearen Gleichungsystemen.
			\begin{enumcols}[3]
...
\section{Weitere Gleichungssysteme}
	Der Begriﬀ des Gleichungssystems ist ein sehr allgemeiner. Zu Beginn haben wir uns immer auf lineare $2\times2$ Gleichungssysteme eingeschrankt. Diese Einschränkung wollen wir an dieser Stelle öffnen und auch weitere Möglichkeiten diskutieren. Zuerst betrachten wir lineare $3\times3$ Gleichungssysteme. Danach arbeiten wir mit linearen Gleichungssystemen mit Parametern und zum Schluss  Wir schauen uns in diesem Abschnitt ein Beispiel eines nicht-linearen Gleichungssystems an.

\subsection{Gleichungssysteme mit drei und mehr Unbekannten}
	Ein lineares Gleichungssystem mit drei Variablen benötigt auch drei Gleichungen, um eindeutigt gelöst werden zu können. Wenn wir nun drei lineare Gleichungen in den drei Unbekannten $x,y$ und $z$ betrachten, versuchen wir die Lösung dieses Gleichungssystems auf die Lösung eines Gleichungssystems nur zweier linearer Gleichungen mit nur zwei Unbekannten zurückzuführen.\vspace{5pt}\newline
...
\subsection{Lineare Gleichungssysteme mit Parametern}
	Kommen neben den Lösungsvariablen noch weitere Variablen vor, so nennt man diese \textbf{Parameter}. Wenn nichts anderes geschrieben ist, werden die Lösungsvariablen mit $x$,$y$, ($z$) bezeichnet; wohin gegeb die Parameter häufig mit $a$, $b$ usw. bezeichnet werden.\vspace{11pt}\newline
...
\subsection{Nicht-lineare Gleichungssysteme}
	Wie wir beim Eisetzungsverfahren gesehen haben, eignet sich dieses vor allem für kompliziertere, \textbf{nicht-lineare} Gleichungssysteme. Jedoch haben wir noch nicht diskutiert, wie ein solches Gleichungssystem aussehen wird. Deshalb starten wir hier mit einem Beispiel.
	\begin{beispiel}
		\[\left\{
		\begin{matrix}
			x^2+y^2=25\\
...
\section{Textaufgaben}
	...
	\begin{aufgabe}
		\begin{enumerate}[label=\alph*)]
			...
			\item Von zwei Eisenbahnstationen, deren Entfernung $d$ Meter beträgt, fahren gleichzeitig zwei Züge $A,B$ ab. Jeder fährt mit konstanter Geschwindigkeit. Fahren sie einander entgegen, so treﬀen sie sich nach $4$ Minuten, fahren sie in dieselbe Richtung, so erst nach $20$ Minuten ($A$ ist
			der schnellere Zug). Bestimme die Geschwindigkeit $v_A$ und $v_B$ der Züge.
		\end{enumerate}
	\end{aufgabe}

\subsection{Vertiefung Mischaufgaben}
