	...
	\begin{beispiel}\ \\
		\begin{minipage}[t]{0.32\linewidth}
			\begin{enumerate}[label=\alph*)]
				\item Schreibe $6^3=216$ in logarithmischer Form.\\ \emph{Lösung:}\[6^3=216 \Leftrightarrow \log{6}{216}=3\]
			\end{enumerate}
		\end{minipage}
		\hfill
		\begin{minipage}[t]{0.32\linewidth}
			\begin{enumerate}[label=\alph*)]
				\setcounter{enumi}{1}
				\item Schreibe $\log{5}{625}=4$ in exponentieller Form.\\\emph{Lösung:}\[\log{5}{625}=4 \Leftrightarrow 5^4=625\]
			\end{enumerate}
		\end{minipage}
		\hfill
		\begin{minipage}[t]{0.32\linewidth}
			\begin{enumerate}[label=\alph*)]
				\setcounter{enumi}{2}
				\item Löse $7^x=230$ auf drei Nachkommastellen.\\\emph{Lösung:}
				\begin{align*}
...
\section{Rechenregeln}
	...
	\begin{satz}
		Auch hier gilt $a>0$, $a\neq1$ und $b>0$. $r$ sei eine beliebige Zahl ($r\in\R$).
		\begin{enumerate}
			\item $a^{\log{a}{b}}=b$
			\item $\log{a}{a^r}=r$
			\item $\log{a}{1}=0$
...
\subsection{Vergleich von Zahlen}
	...
	\end{aufgabe}
	Wie kann ich nun rechnerisch überprüfen, welcher dieser Potenzen grösser ist?\vspace{5pt}\\\karos{4}
	\begin{aufgabe}
		\begin{enumerate}[label=\alph*)]
