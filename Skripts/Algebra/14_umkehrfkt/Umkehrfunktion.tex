...
\newpage
	...
	\end{definition}
	\begin{aufgabe}
		Entschiede, ob die folgenden Funktionen bijektiv sind oder nicht.\newline
		\begin{minipage}{0.45\linewidth}
			\begin{enumerate}[label=\alph*)]
				\item $f(x)=\sqrt{9-x^2}$ für $\D=\left[-3,3\right]$ und $\W=\R^+_0$
				\item $g(x)=x^2+2x$ für $\D=\left[-1,\infty\right]$ und $\W=\left[-1,\infty\right]$
				\item $h(x)=\frac{2}{x}$ für $\D=\W=\R$
				\item $j(x)=\frac{1}{5}\sqrt{2x+4}$ für $\D=\left[-2,\infty\right]$ und $\W=\R_0^+$
			\end{enumerate}
		\end{minipage}
		\hfill
		\begin{minipage}{0.45\linewidth}
...
\subsection{Wie berechnet man eine Umkehrfunktion?}
	...
	\begin{beispiel}
		Sei die lineare Funktion \[f:y=\frac{2x-5}{4}\] gegeben. Lasst uns zuerst überprüfen, ob die Funktion auch wirklich umkehrbar ist.\vspace{5pt}\newline\karos{5.5}

		Die Funktionsgleichung zeigt uns, nach welcher Formel dem Argument $x$ einer Zahl  $y$ zugeordnet wird. Da die Umkehrfunktion gerade das Umgekehrte tut, also einem $y-$Wert sein eindeutiges Argument $x$ zuweist, müssen wir uns fragen, nach welcher Formel ein vorgegebenes $y$ in $x$ umgewandelt wird. Mit anderen Worten: Wir müssen die Funktionsgleichung einfach nach $x$ auflösen:\vspace{5pt}\newline\karos{4.5}
	\end{beispiel}
	\begin{aufgabe}
		\begin{enumerate}
			\item Berechne für die folgenden Funktionen $f$ ihre Umkehrfunktion $f^{-1}$, sofern diese existiert.
			\begin{enumcols}[2]
				\item $f_1:\R\to\R,\:y=\frac{1}{4}x$
				\item $f_2:\R_0^+\to\R_0^+,\:y=x^3$
				\item $f_3:\R\to\R,\:y=3x+2$
				\item $f_4:\R\to\R,\:y=\frac{3x+7}{4}$
				\item $f_5:\R_{\geq 3}\to\R_0^+,\:y=\sqrt{2x-6}$
				\item $f_6:\R\to\R,\:y=4$
			\end{enumcols}
			\item Von welcher Funktion $f$ mit welcher maximalen Definitionsmenge sind die folgenden Funktionen $g$ die Umkehrfunktion?
			\begin{enumcols}[2]
				\item $g_1:\R_{\geq -5}\to\R_0^+,\:y=\sqrt{x+5}$
				\item $g_2:\R\to\R,\:y=\frac{1}{3}x+8$
				\item $g_3:\R_{\geq\frac{1}{4}}\to\R_0^+,\:y=\sqrt{4x-1}$
				\item $g_4:\R\setminus\left\{2\right\}\to\R,\:y=\frac{3}{x-2}$
			\end{enumcols}
		\end{enumerate}
	\end{aufgabe}

\subsection{Was tun, wenn $f$ nicht bijektiv ist?}
