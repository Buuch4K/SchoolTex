...
\subsection*{Erinnerung: Was ist eine Funktion?}
	...
	\begin{definition}
		...
		\begin{figure}[H]
			\centering\includegraphics[scale=0.6]{RepFunktion.png}
		\end{figure}
	\end{definition}
...
\section{Definition der Quadratischen Funktion}
	...
	\begin{figure}[h]
		\begin{minipage}{0.3\linewidth}
			\centering\includegraphics[width=4cm]{brunnen.png}
		\end{minipage}
		\hfill
		\begin{minipage}{0.3\linewidth}
			\centering\includegraphics[width=4cm]{gatewayarch.png}
		\end{minipage}
		\hfill
		\begin{minipage}{0.3\linewidth}
			\centering\includegraphics[width=4cm]{brücke.png}
		\end{minipage}
	\end{figure}
	...
	\begin{definition}[Quadratische Funktionen]
		Eine Funktion, deren Funktionsgleichung in die Form \[\mathbf{f:y=a\*x^2+b\*x+c,}\] gebracht werden kann, wobei $a,b,c\in\R$ und $a\neq0$ gilt, heisst \textbf{quadratische Funktion}.\newline
...
\section{Die Parabel}
	...
	\end{aufgabe}
	\begin{definition}
		Der Graph einer quadratischen Funktion heisst \textbf{Parabel}. Die Funktionsgleichung hat dabei immer die Form \[f:y=a\*x^2+b\*x+c\quad(a\neq0).\] In seiner einfachsten Form $f:y=x^2$  nennt man den Graph \textbf{Normalparabel}.
	\end{definition}
...
\subsection{Eigenschaften der Parabel}
	...
	\begin{minipage}{0.475\linewidth}
		...
		\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=10mm, y=10mm,
			...
			\draw[forestgreen,thick,samples=100] plot (\x, \x*\x);
			%Beschriftung
			\node[forestgreen]  at (2.8,6.8) {\normalsize$f$};
		\end{tikzpicture}
	\end{minipage}
...
\subsubsection*{$f(x)=\mathbf{a}\*x^2$}
	...
	\begin{table}[ht]
		\centering
		\begin{tabular}{cll}
			\multirow{4}{*}{\includegraphics[width=4cm]{images/streckung.png}} & $a$ bewirkt& \gap{7.5} \\
			& $a>1$:  & \gap{7.5} \\
			& $0<a<1$: & \gap{7.5} \\
			& $a<0$: & \gap{7.5} \\
		\end{tabular}
...
\subsubsection*{$f(x)=x^2\mathbf{+v}$}
	\begin{table}[ht]
		\centering
		\begin{tabular}{cll}
			\multirow{3}{*}{\includegraphics[width=4cm]{images/vertikaleverschiebung.png}} & $v$ bewirkt& \gap{7.5}\\
			& $v>0$:  & \gap{7.5}\\
			& $v<0$: & \gap{7.5}
		\end{tabular}
	\end{table}
...
\subsubsection*{$f(x)= (x\mathbf{-u})^2$}
	\begin{table}[ht]
		\centering
		\begin{tabular}{cll}
			\multirow{3}{*}{\includegraphics[width=4cm]{images/horizontaleverschiebung.png}} & $u$ bewirkt& \gap{7.5}\\
			& $u>0$:  & \gap{7.5}\\
			& $u<0$: & \gap{7.5}
		\end{tabular}
	\end{table}
...
\subsection{Von der Normalform zur Scheitelpunktform}
	...
	\begin{aufgabe}
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
				...
				% Achsen beschriften
				\foreach \x in {-20,-15,-10,-5,5,10,15,20} \draw (\x,-.2) -- (\x,.2) node[below=4pt] {$\x$};
				\foreach \y in {-20,-15,-10,-5,5,10,15,20} \draw (-.2,\y) -- (.2,\y) node[left=4pt] {$\y$};
				% Besserer Ursprung
				\draw[color=black] (0pt,-20pt) node[right] {$0$};
				%Funktionen zeichnen:
				\clip(-20.3,-20.3) rectangle (20.8,20.8); %Bildbereich
				\draw[highdive,thick,samples=100] plot (\x, {pow(\x,2)});
				%Beschriftung
	...
	\begin{aufgabe}
		...
		\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
			...
			font= \footnotesize, scale=0.5]
			%Raster zeichnen
			\draw [color=gray]  [step=10mm] (-25,-6) grid (7,7);
			% Achsen zeichnen
			...
			\draw[highdive,thick,samples=100] plot (\x, {2*pow(\x-3,2)-2});
			\draw[forestgreen,thick,samples=100] plot (\x, {-1*pow(\x+2,2)+4});
			\draw[plotblue,thick,samples=100] plot (\x, {pow(\x-2,2)+2});
			\draw[myred,thick,samples=100] plot (\x, {-0.5*pow(\x,2)+1});
			%Beschriftung
...
\subsection*{Beispiel: Rakete}
	...
	\begin{table}[H]
		\centering
		\begin{tabular}{|p{2cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|p{0.7cm}|}\hline
	...
	\begin{figure}[ht]
		\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1mm,
			...
			font=\footnotesize, scale=0.5]
			%Raster zeichnen
			\draw [color=gray!50]  [step=10mm] (-5,-30.1) grid (29,180);
			% Achsen zeichnen
			\draw[->,thick] (-4.3,0) -- (12.8,0) node[right, below] {\normalsize $x$};
			\draw[->,thick] (0,-23) -- (0,168) node[above, left] {\normalsize $y$};
			% Achsen beschriften
			\foreach \x in {-4,-2,2,4,...,12} \draw (\x,-1) -- (\x,1) node[below=4pt] {$\x$};
			\foreach \y in {-20,20,40,...,160} \draw (-.1,\y) -- (.1,\y) node[left=4pt] {$\y$};
			% Besserer Ursprung
			\draw[color=black] (0pt,-20pt) node[right] {$0$};
			%Funktionen zeichnen:
			\clip(-2,-20) rectangle (12,165); %Bildbereich
	...
	\begin{aufgabe}[Weitere Beispiele]
		\begin{enumerate}
			\item Die Leistung $P$ einer Schiﬀsturbine hängt von der Anzahl $n$ der Umdrehungen pro Minute ab. Die Funktion mit der Gleichung $P(n)=-0.75n^2+330n$ gibt die Leistung in Watt an.
			\begin{enumerate}[label=\alph*)]
				\item Berechne die folgenden Leistungen der Turbine: $P(100), P(150), P(300), P(340)$.
				\item Für welches n ist die Turbinenleistung maximal? Wie gross ist sie?
			\end{enumerate}
			\item  Noel springt im Freibad vom Sprungbrett. Seine Flugbahn entspricht ungefähr einer Parabel mit der Funktionsgleichung $h(x)=-5x^2+2x+3$. Hierbei ist $h$ die Höhe über dem Wasser (in $m$) und $x$ die horizontale Entfernung vom Absprungpunkt (in $m$).
			\begin{enumerate}[label=\alph*)]
				\item Von welcher Höhe ist Noel abgesprungen?
				\item Was ist Noels grösste Höhe während des Fluges?
				\item Gib eine Funktionsgleichung für einen Sprung aus einer anderen Höhe an.
			\end{enumerate}
			\item Ein Brückenbogen hat die Form einer Parabel. Die Scheitelhöhe des Bogens beträgt $4.5$m, seine Spannweite $10$m.
			\begin{enumerate}[label=\alph*)]
				\item Die Schweizer Normen für Fahrzeuge sehen eine maximale Breite von $2.55$m und eine maximale Höhe von $4$m vor. Kann ein Sattelschlepper mit diesen Massen unter dem Brückenbogen hindurchfahren?
				\item Welche maximale Breite könnte ein $4$m hohes Fahrzeug haben?
				\item Kann der Sattelschlepper aus a) einen $2.14$m breiten und $1.65$m hohen Jeep unter dieser Brücke kreuzen, wenn die beiden seitlich $1$m Abstand haben sollen?
			\end{enumerate}
			\item Der Wasserstrahl aus einem Gartenschlauch hat etwa die Form einer Parabel mit der Gleichung $p(x)=a\*x^2$. Wir wissen, dass der Strahl nach $1.5$m um $7.5$m gefallen ist.
			\begin{enumerate}[label=\alph*)]
				\item Der Strahl trifft $6$m vor den Füssen der Person, die den Schlauch hält, entfernt auf den Boden. Wie hoch wird die Schlauchspitze gehalten?
				\item Wie hoch muss der Schlauch gehalten werden, damit der Wasserstrahl $7.5$m weit reicht?
				\item Wie weit reicht der Strahl, wenn die Person zusätzlich auf einer $3$m hohen Leiter steht?
			\end{enumerate}
		\end{enumerate}
	\end{aufgabe}
...
\subsection{Schnittpunkte von Parabel mit Gerade}
	...
	\begin{minipage}{0.3\linewidth}
		...
		\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
			...
			\draw[highdive,thick,domain=-2:7,samples=100] plot (\x, {-pow(\x,2)+5*\x});
			\draw[highdive,thick,domain=-3.3:7.3,samples=100] plot (\x,{-2/3*\x+5});
			%Beschriftung
			%\node[highdive]  at (5,18) {$p$};
		\end{tikzpicture}
	...
	\begin{minipage}{0.3\linewidth}
		...
		\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
			...
			\draw[highdive,thick,domain=-9.3:1.3,samples=100] plot (\x, {-0.5*pow(\x,2)-4*\x-3});
			\draw[highdive,thick,domain=-9.3:1.3,samples=100] plot (\x,{2*\x+15});
			%Beschriftung
			%\node[highdive]  at (5,18) {$p$};
		\end{tikzpicture}
		Diskriminante $\mathbf{D=0}$
	...
	\begin{minipage}{0.3\linewidth}
		...
		\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
			...
			%Funktionen zeichnen:
			\clip(-3.3,-8.3) rectangle (7.3,8.3); %Bildbereich
			\draw[highdive,thick,domain=-3.3:7.3,samples=100] plot (\x, {3*pow(\x,2)-6*\x+4});
			\draw[highdive,thick,domain=-3.3:7.3,samples=100] plot (\x,{\x-5});
			%Beschriftung
			%\node[highdive]  at (5,18) {$p$};
		\end{tikzpicture}
		Diskriminante $\mathbf{D<0}$
	...
	\begin{aufgabe}
		...
		\begin{enumcols}[2]
			...
			\item $\displaystyle p:y=-x^2+5x \text{ und } g:y=-\frac{2}{3}x+5$
			\item $\displaystyle p:y=-\frac{1}{2}x^2-4x-3 \text{ und } g:y=2x+15$
		\end{enumcols}
	\end{aufgabe}
	\begin{aufgabe}
...
\subsection{Schnittpunkte von zwei Parabeln}
	...
	\begin{beispiel}
		...
		\begin{figure}[H]
			\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
				...
				font=\footnotesize, scale=0.5]
				%Raster zeichnen
				\draw [color=gray!50]  [step=10mm] (-7,-7) grid (27,13);
				% Achsen zeichnen
				...
				\draw[highdive,thick,domain=-5:10,samples=100] plot (\x, {-0.5*pow(\x,2)+3*\x+4.5});
				\draw[highdive,thick,samples=100] plot (\x,{pow(\x,2)});
				%Beschriftung
				%\node[highdive]  at (5,18) {$p$};
			\end{tikzpicture}
	...
	\begin{aufgabe}
		\begin{enumerate}
			\item Berechne die Schnittpunkte der beiden Parabeln.
			\begin{enumerate}[label=\alph*)]
			...
			\end{enumerate}
			\item Bestimme den Wert des Parameters so, dass die Gerade $g$ Tangente an die Parabel mit der Gleichung $p:y=x^2-3$ ist. Berechne die Koordinaten des Berührungspunktes.
			\begin{enumcols}[3]
				\item $\displaystyle g:y=q$
				\item $\displaystyle g:y=2x+q$
				\item $\displaystyle g:y=mx-9.25$
			\end{enumcols}
		\end{enumerate}
	\end{aufgabe}
...
\section{Funktionsgleichung bestimmen}
	...
	Wir stelle die drei Gleichungen auf und löse anschliessend dieses Gleichungssystem mithilfe des Taschenrechners. Dazu verwenden wir folgende Tasten:\newline
	{\bf Menu $->$ 3 $->$ 7 $->$ 1}\vspace*{5pt}\newline\karos{4.5}
	\newpage
	\begin{aufgabe}
		Bestimme die Gleichung der beschriebenen Parabel in der Form $f:y = ax^2 + bx + c$. Berechne auch die Koordinaten des Scheitels $S$.
		\begin{enumerate}[label=\alph*)]
			\item Eine nach oben geöﬀnete Normalparabel, die durch die Punkte $A(0\:\mid\:4)$ und $B(9\:\mid\:31)$ geht.
			\item Eine nach unten geöﬀnete Normalparabel mit den Nullstellen $x_1 = 4$ und $x_2 =-2$.
			\item Eine Parabel mit dem Scheitelpunkt $S(5\:\mid\:1)$, die den Punkt $R(-10\:\mid\:136)$ enthält.
			\item Eine Parabel, auf der die Punkte $A(4\:\mid\:0)$, $B(6\:\mid\:-9)$ und $C(-4\:\mid\:-24)$ liegen.
			\item Eine Parabel mit dem Scheitel $S(3\:\mid\:-5)$, die den y-Achsenabschnitt $-\frac{1}{2}$ besitzt.
		\end{enumerate}
	\end{aufgabe}
	\begin{aufgabe}
		Betrachten den folgenden Graphen und bestimme darin alle Funktionsgleichungen der abgebildeten Parabeln.
		\begin{figure}[H]
			\centering
			\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
				>=latex,   %Voreinstellung für Pfeilspitzen
				font=\footnotesize, scale=0.5]
				%Raster zeichnen
				\draw [color=gray]  [step=10mm] (-6,-20) grid (26,8);
				% Achsen zeichnen
				\draw[->,thick] (-5.3,0) -- (5.8,0) node[right, below] {\normalsize $x$};
				\draw[->,thick] (0,-3.3) -- (0,6.8) node[above, left] {\normalsize $y$};
				% Achsen beschriften
				\foreach \x in {-5,...,-2,-1,1,2,...,5} \draw (\x,-.1) -- (\x,.1) node[below=4pt] {$\x$};
				\foreach \y in {-3,-2,-1,1,2,...,6} \draw (-.1,\y) -- (.1,\y) node[left=4pt] {$\y$};
				% Besserer Ursprung
				\draw[color=black] (-0.25,-0.35) node {$0$};
				%Funktionen zeichnen:
				\clip(-6.3,-3.3) rectangle (6.3,6.3); %Bildbereich
				\draw[forestgreen,thick,domain=-5.3:5.3,samples=100] plot (\x, {(\x)^2-4*\x+5});
				\draw[highdive,thick,domain=0:6.3,samples=100] plot (\x, {2*(\x)^2-16*\x+29});
				\draw[myred,thick,domain=-6.3:1,samples=100] plot (\x, {-0.5*(\x)^2-3*\x-2.5});
				%Beschriftung
				\node[forestgreen]  at (4.7,5.5) {$f$};
				\node[highdive]  at (5.3,-1.5) {$g$};
				\node[myred]  at (-4.5,1.5) {$h$};
			\end{tikzpicture}
		\end{figure}
	\end{aufgabe}
...
\section{Extremwertaufgaben}
	\begin{minipage}{0.4\linewidth}
		Parabeln haben entweder ein Minimum oder ein Maximum. Das Min./Max. ist die y-Koordinate des Scheitelpunktes. Macht eine quadratische Funktion eine Aussageüber einen Sachverhalt, ist oft das Minimum und Maximum von besonderem Interesse!
	\end{minipage}
	\hfill
	\begin{minipage}{0.28\linewidth}
		\centering
		\includegraphics[width=4cm]{minimum.png}
	\end{minipage}
	\hfill
	\begin{minipage}{0.28\linewidth}
		\centering
		\includegraphics[width=4cm]{maximum.png}
	\end{minipage}
	\begin{beispiel}
		Mit einem $12$m langen Zaun soll an einer Hauswand ein Rechteck eingezäunt werden. Wie lang müssen die Seiten des Rechtecks sein, damit sein Flächeninhalt möglichst gross wird?\vspace{11pt}\newline
		\begin{minipage}{0.68\linewidth}
			\begin{enumerate}
				\item Variablen einführen:\newline \[x=\gap{3}\text{ und } z=\gap{3}\]
				\item Definitionsbereich angeben:\newline \[\D_x=\gap{3}\text{ und } \D_z=\gap{3}\]
				\item Zusammenhang zwischen Variablen finden/ Gleichung aufstellen:\newline Nebenbedingung: \gap{7.4}
				\item Zu maximierende (oder minimierende) Grösse in Abhängigkeit einer Variable formulieren:\newline Zielfunktion: \gap{8.2}\vspace{1.5cm}
				\item Scheitelpunkt der Zielfunktion berechnen:
			\end{enumerate}\karos{2.5}
			\begin{enumerate}
				\setcounter{enumi}{5}
				\item Überprüfen, ob der Extremalwert innerhalb des Definitionsbereichs liegt und Fragestellung beantworten:
			\end{enumerate}
		\end{minipage}
		\hfill
		\begin{minipage}{0.28\linewidth}
			{\bf Skizze der Situation}\newline
			\includegraphics[width=3.5cm]{extrem-zaun.png}
			\vspace*{1cm}\newline
			{\bf Graph der Zielfunktion}\newline
			\includegraphics[width=4.5cm]{extrem-zaun-graph.png}
			\vspace*{1.5cm}
		\end{minipage}
		\vspace{5pt}\newline\karos{3}
	\end{beispiel}
	\begin{aufgabe}
		\begin{enumerate}
			\item Berechne das Minimum bzw. das Maximum, das der Term annehmen kann.
			\begin{enumcols}[2]
				\item $\displaystyle x^2-8x+25$
				\item $\displaystyle x^2+x-18$
				\item $\displaystyle -x^2+8x+3$
				\item $\displaystyle 5-x-\frac{1}{2}x^2$
			\end{enumcols}
			\item Wie muss die Strecke $a=4$cm in zwei Teilstrecken $u$ und $v$ zerlegt werden, damit die Summe der Flächeninhalte, der über den beiden Teilstrecken errichteten Quadrate kleinstmöglich wird?
			\item Wähle mehrmals zwei positive Zahlen mit der Summe $140$. Bilde sowohl ihr Produkt als auch die Summe der Quadrate dieser Zahlen.
			\begin{enumerate}[label=\alph*)]
				\item Für welche zwei Zahlen wird das Produkt maximal? Begründe.
				\item Für welche zwei Zahlen wird die Summe ihrer Quadrate minimal? Begründe.
			\end{enumerate}
			\item Für welche Zahl wird das Produkt aus dem Dreifachen und der um $1$ vergrösserten Zahl am kleinsten?
			\item Welche beiden Zahlen, deren Differenz $2$ beträgt, haben das kleinste Produkt?
			\item Max will mit $11$m Maschendraht einen rechteckigen Platz für seinen Hund einzäunen.
			\begin{enumerate}[label=\alph*)]
				\item Er benutzt die Wände der Garage und des Hauses (Abbildung links). Für welche Abmessungen wird der Platz am grössten? Gib den Flächeninhalt an.
				\item Wie hätte er die Abmessungen wählen müssen, wenn er eine $3$m lange Mauer einbeziehen würde (Abbildung rechts)?
			\end{enumerate}
			\begin{center}
				\begin{minipage}{0.3\linewidth}
					\centering
					\includegraphics[scale=0.4]{extremal-hausgarage.png}
					\label{fig:hausgarage}
				\end{minipage}
				\begin{minipage}{0.3\linewidth}
					\centering
					\includegraphics[scale=0.4]{extremal-mauer.png}
					\label{fig:mauer}
				\end{minipage}
			\end{center}
			\item Auf den Seiten des Rechtecks $ABCD$ wird auf jeder Seite die Strecke $x$ abgetragen. Es entsteht das Viereck $EFGH$.
			\begin{enumerate}[label=\alph*)]
				\item Welche Form hat das Viereck $EFGH$? Begründe.
				\item Für welche Länge $x$ ist der Flächeninhalt des Vierecks $EFGH$ am kleinsten?\newline Tipp: Überlege, welche Flächen von der Fläche des Rechtecks abgezogen werden müssen, um auf das Viereck $EFGH$ zu kommen.
			\end{enumerate}
			\begin{figure}[H]
				\centering\includegraphics[scale=0.5]{extremal-viereck.png}
			\end{figure}
		\end{enumerate}
	\end{aufgabe}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{13mm}|p{20mm}|p{30mm}|p{70mm}|}\hline
		...
		$1.1$ & 15.02.2026 & Jonas Guler & Fktgl bestimmen und Extremal Theorieinputs ergänzt\\\hline
		$1.2$ & 17.03.2026 & Jonas Guler & Aufgaben Extremal ergänzt und einige Korrekturen\\\hline
	\end{tabular}
\end{table}
