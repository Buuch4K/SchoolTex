	...
	\begin{aufgabe}
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=1cm, y=1cm,
				...
				font=\footnotesize, scale=0.5]
				%Raster zeichnen
				\draw [color=gray]  [step=10mm] (-6,-13) grid (26,8);
				% Achsen zeichnen
				\draw[->,thick] (-5.3,0) -- (5.8,0) node[right, below] {\normalsize $x$};
				...
				% Besserer Ursprung
				\draw[color=black] (0pt,-2pt) node[right] {$0$};
				%Funktionen zeichnen:
				\clip(-6,-3.3) rectangle (6,6.3); %Bildbereich
				\draw[forestgreen,thick,domain=-5.3:5.3,samples=100] plot (\x, {(\x)^2-4*\x+5});
				\draw[highdive,thick,domain=-5.3:5.3,samples=100] plot (\x, {2*(\x)^2-16*\x+29});
				\draw[myred,thick,domain=-5.3:5.3,samples=100] plot (\x, {-0.5*(\x)^2-3*\x-2.5});
				%Beschriftung
...
\section{Modellieren}
	...
	\begin{aufgabe}
		\begin{enumerate}
			...
			\begin{enumerate}[label=\alph*)]
				\item Die Schweizer Normen für Fahrzeuge sehen eine maximale Breite von $2.55$m und eine
				maximale Höhe von $4$m vor. Kann ein Sattelschlepper mit diesen Massen unter dem
				Brückenbogen hindurchfahren?
				\item Welche maximale Breite könnte ein $4$m hohes Fahrzeug haben?
				\item Kann der Sattelschlepper aus a) einen $2.14$m breiten und $1.65$m hohen Jeep unter
				dieser Brücke kreuzen, wenn die beiden seitlich $1$m Abstand haben sollen?
			\end{enumerate}
...
\begin{table}[H]
	...
	\begin{tabular}{|p{13mm}|p{20mm}|p{30mm}|p{70mm}|}\hline
		...
		$1.1$ & 15.02.2026 & Jonas Guler & Fktgl bestimmen und Extremal Theorieinputs ergänzt - Aufgaben und Modellieren fehlen  \\\hline
	\end{tabular}
\end{table}
