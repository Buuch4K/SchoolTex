...
\begin{minipage}[t]{0.5\linewidth}
	Die Graphen welcher quadratischer Funktionen sind im Graphen rechts abgebildet? Gib die jeweiligen Funktionsgleichungen an, wobei du \textbf{nur} die grau eingezeichneten Punkte verwenden darfst!
\end{minipage}
\hfill
...
\begin{minipage}[t]{0.5\linewidth}
	Bestimme die Gleichungen dieser Parabeln und bringe sie in die Form $y=ax^2+bx+c$. Du darfst dabei \textbf{nur} die grau markierten Punkte verwenden!
\end{minipage}
\hfill
\begin{minipage}[t]{0.475\linewidth}
	\centering\vspace{-0.7\baselineskip}
...
%-------------------------------------------------------
\noaddpoints
\question[\totalpoints] %GZP22 A14
\addpoints
Ein Ball wird über eine 8 m hohe Mauer geschossen. Seine Flugbahn kann näherungsweise durch den Graphen der Funktion $f(x)=-0.4x^2+4.8x-4.4$ beschrieben werden.\\
\begin{minipage}[t]{0.6\linewidth}
	\begin{parts}
		\part[1\half] Bestimme die Koordinaten des Punktes $A$, von welchem aus der Ball geschossen wird.
		\part[1] Die Mauer steht bei $x=4$m. Wie viele cm über der Mauer wird der Ball fliegen?
		\part[1] Berechne die Koordinaten des höchsten Punkts $C$ auf der Flugbahn des Balles.\vspace*{7pt}
	\end{parts}
\end{minipage}
\hfill
\begin{minipage}[t]{0.375\linewidth}
	\centering\vspace{-0.7\baselineskip}
	\includegraphics[scale=0.75]{quadfunk-mauerfussball.png}
\end{minipage}
\begin{parts}
	\setcounter{partno}{3}
	\part[1\half] Berechne den Punkt $D$, auf welchem der Ball hinter der Mauer auftrifft, wenn dort der Boden um $2$m höher ist als vor der Mauer (siehe Skizze).
\end{parts}
