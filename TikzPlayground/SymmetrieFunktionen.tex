...
\begin{document}
	\begin{tikzpicture}[cross/.style={draw, cross out, minimum size=2*(#1-1pt), inner sep=0pt, outer sep=0pt}, x=10mm, y=10mm,
		...
		\draw[myred,thick,domain=-2:2,samples=200] plot (\x, {2*(\x)^5-6*(\x)^3+1.8*\x});
		\draw[plotblue,thick,domain=-2:2,samples=200] plot (\x, {(\x)^3-2*(\x)});
		\draw[forestgreen,thick,domain=-3:3,samples=200] plot (\x, {(\x)^4-3*(\x)^2-1});
		\draw[highdive,thick,domain=-2:2,samples=200] plot (\x, {(\x)^6-2*(\x)^4+0.5*(\x)^2+1});
		%Beschriftung
